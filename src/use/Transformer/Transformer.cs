...
public sealed class Transformer
{
    ...
    readonly Dictionary<string, int> _encoder;
    readonly Dictionary<int, string> _decoder;
    ...
    Transformer(InferenceSession model, Dictionary<int, string> decoder)
    {
        _model = model;
        _decoder = decoder;
        _encoder = _decoder.ToDictionary(x => x.Value, x => x.Key);
    ...
    public IEnumerable<string> Continue(string[] previousTokens)
    {
        var context = Enumerable.Repeat(0, Math.Max(SampleSize - previousTokens.Length, 0))
            .Concat(previousTokens.TakeLast(SampleSize).Select(s => _encoder[s])).ToArray();
        ...
        for (; ;)
        {
            ...
            var distribution = new double[_decoder.Count];
            for (var i = 0; i < _decoder.Count; i++)
